    ...
    <div className="space-y-6">
      ...
      {overall.total > 0 ? (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <StatCard label="Hands Played" value={String(overall.total)} />
          <StatCard label="Accuracy" value={`${overall.accuracy}%`} />
          <StatCard label="Avg. Mistake Cost" value={`${overall.avgEvLoss.toFixed(2)} bets`} />
          <StatCard label="Avg. Time" value={`${(overall.avgDecisionTimeMs / 1000).toFixed(1)}s`} />
        </div>
      ) : (
        <div className="text-muted-foreground text-sm text-center py-8">
          No training data yet. Head to the trainer and play some hands!
        </div>
      )}
