...
export function SessionHud() {
  ...
  const accuracy = Math.round((correctCount / handsPlayed) * 100)
  const avgTimeS = (totalDecisionTimeMs / handsPlayed / 1000).toFixed(1)
  const elapsedMinutes = totalDecisionTimeMs / 60_000
  const handsPerMinute = elapsedMinutes > 0 ? (handsPlayed / elapsedMinutes).toFixed(1) : '—'
  ...
  return (
    <div className="flex items-center justify-center gap-4 rounded-lg bg-neutral-900/50 px-4 py-2 text-xs">
      ...
      <Stat label="Avg time" value={`${avgTimeS}s`} />
